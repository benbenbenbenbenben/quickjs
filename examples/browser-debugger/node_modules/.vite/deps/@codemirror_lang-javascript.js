...
var bracketMatchingState = StateField.define({
  ...
  update(deco, tr) {
    ...
    for (let range of tr.state.selection.ranges) {
      ...
      if (match)
        decorations = decorations.concat(config2.renderMatch(match, tr.state));
    }
...
function clipRTLLines(ranges, doc) {
  let cur2 = doc.iter(), pos = 0, result = [], last = null;
  for (let { from, to } of ranges) {
...
var autoCloseTags = EditorView.inputHandler.of((view, from, to, text, defaultInsert) => {
  ...
  let closeTags = state.changeByRange((range) => {
    ...
    if (state.doc.sliceString(head - 1, head) != text || around.name == "JSXAttributeValue" && around.to > head) ;
      ...
      if (base2 && empty.from == head - 2 && ((name2 = elementName(state.doc, base2.firstChild, head)) || ((_a2 = base2.firstChild) === null || _a2 === void 0 ? void 0 : _a2.name) == "JSXFragmentTag")) {
        let insert = `${name2}>`;
